...
class lora_sqlite(object):
    ...
    def update_data_lite(self, date, ID, value):
        ...
        self.con.commit()
    def field_lite(self, date, ID, snr, rssi):
        db=self.con.cursor()
        db.execute('INSERT INTO fieldTest (timeRecorded, SensorID, snr, rssi) VALUES (?,?,?,?)', (date, ID, snr, rssi))
        self.con.commit()
...
class lora_my(object):
    ...
    def update_mySQL(self, data):
        ...
        if self.connectFlag:
            ...
            for item in data:
                insert_sensor_data = ("INSERT INTO Data1" "(timeRecorded, SensorID, Value)" "VALUES (%s, %s, %s)")
